...
const usersThatHavePostedOnlyAboutPHP = filter(users, user => {
    const postsOnlyAboutPHP = users.posts.filter(post => post.tags.includes('PHP') &&
    post.tags.length == 1);
    return usersThatHavePostedOnlyAboutPHP.length > 0;
});