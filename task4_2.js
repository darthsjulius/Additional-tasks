const users = [
  ...
  const usersThatHaventPostedAboutPython = filter(users, user => {
    const postsWithoutPython = users.posts.filter(post => !post.tags.includes('Python'));
    return postsWithoutPython.length > 0;
  });